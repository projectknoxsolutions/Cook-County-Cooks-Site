...
import { preflight, preflightCopy } from './preflight.aa4603d977.js';
export { preflight };
 ...
 *    custom property with a fallback, so assets/theme.2d7c2c8824.css (Agent D) can
...
function showFrame(tool, { force = false } = {}) {
  ...
  // are in data/tools.ac8a24642f.json — but the list stays as cheap insurance for whatever
...
export function initOverlay(options = {}) {
  ...
  /* ⚠ THE INLINE FALLBACK READ THE WRONG GLOBAL AND SO WAS NEVER A FALLBACK.
     ...
     fetch of data/tools.ac8a24642f.json — which is the one thing the inline payload exists
